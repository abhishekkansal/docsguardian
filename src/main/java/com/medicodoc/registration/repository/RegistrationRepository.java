...
public interface RegistrationRepository extends JpaRepository<RegistrationData, Integer> {
    @Query(value = "SELECT count(*) FROM user_reg_data WHERE username like ?1 and password like ?2 ", nativeQuery = true)
    int validateIdPassword(String username, String password);
